...
# Eksempeldata - du skal erstatte dette med dine egne data
set.seed(123)
personer <- c("Person 1", "Person 2", "Person 3", "Person 4")
projekter <- c("Projekt A", "Projekt B", "Projekt C")
emner <- c("Analyse", "Udvikling", "Test", "Dokumentation")

# Genererer eksempeldata for tidsregistrering
tidsdata <- expand.grid(
  dato = seq(as.Date("2025-01-01"), as.Date("2025-02-28"), by = "day"),
  person = personer,
  projekt = sample(projekter, 4 * 59, replace = TRUE),
  emne = sample(emner, 4 * 59, replace = TRUE)
)

tidsdata$timer <- round(runif(nrow(tidsdata), 1, 8), 1)
tidsdata$start_tid <- format(as.POSIXct("08:00:00", format = "%H:%M:%S") + 
                               sample(0:(3600*3), nrow(tidsdata), replace = TRUE), "%H:%M:%S")
tidsdata$slut_tid <- format(as.POSIXct(tidsdata$start_tid, format = "%H:%M:%S") + 
                              tidsdata$timer * 3600, "%H:%M:%S")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    selectInput("person", "Vælg Person:", 
                choices = c("Alle", personer),
                selected = "Alle"),
    dateRangeInput("datointerval", "Vælg Datointerval:",
                   start = min(tidsdata$dato),
                   end = max(tidsdata$dato)),
    selectInput("projekt", "Vælg Projekt:",
                choices = c("Alle", projekter),
                selected = "Alle"),
    hr(),
    h4("Tidsregistrering"),
    selectInput("reg_person", "Person:", choices = personer),
    selectInput("reg_projekt", "Projekt:", choices = projekter),
    selectInput("reg_emne", "Emne:", choices = emner),
...
server <- function(input, output, session) {
  ...
  # Reaktiv værdi til at holde styr på om der er en tidsregistrering i gang
  time_tracking <- reactiveVal(FALSE)
  current_start_time <- reactiveVal(NULL)
  ...
  upload_to_gsheets <- function(data) {
    ...
    # Dette kræver en engangsgodkendelse
  ...
  all_persons_data <- reactive({
    tidsdata %>%
      filter(dato >= input$datointerval[1] & dato <= input$datointerval[2]) %>%
      group_by(person) %>%
      summarise(total_timer = sum(timer))
  })
  ...
  output$kpi_dashboard_plot <- renderPlot({
    data_projekt <- filtered_data() %>%
      group_by(projekt) %>%
      summarise(timer = sum(timer))
    
    data_person <- filtered_data() %>%
      group_by(person) %>%
      summarise(timer = sum(timer))
    
    data_emner <- filtered_data() %>%
      group_by(emne) %>%
      summarise(timer = sum(timer))
  ...
  output$tidsplot <- renderPlot({
    data <- filtered_data() %>%
      ...
      summarise(timer = sum(timer), .groups = "drop")
  ...
  observeEvent(input$gem_tid, {
    ...
    # Tilføj til eksisterende datasæt
    tidsdata <<- rbind(tidsdata, new_entry)
    
    # Upload til Google Sheets
    result <- tryCatch({
      # Dette er en placeholder - i den faktiske implementering skal du bruge upload_to_gsheets funktionen
      # Kommenteret ud for at undgå fejl, da du skal bruge dine egne Google Sheets indstillinger
      # upload_to_gsheets(new_entry)
      TRUE  # Antag success for eksempelets skyld
    }, error = function(e) {
      showNotification(paste("Fejl ved upload til Google Sheets:", e$message), 
                       type = "error")
      FALSE
    ...
    if (result) {
      showNotification("Tidsregistrering gemt og synkroniseret med Google Sheets", 
                       type = "success")
